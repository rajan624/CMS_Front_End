...
const TextEditor = () => {
  ...
  const handleImageUpload = (file) => {
    setLoading(true);
      return new Promise(async(resolve, reject) => {
        ...
        setLoading(false);
        console.log("🚀 ~ file: TextEditor.js:46 ~ returnnewPromise ~ tempUrl:", tempUrl)       
  ...
  const handelSubmit = async (data, event) => {
    ...
    console.log("🚀 ~ file: TextEditor.js:65 ~ handelSubmit ~ buttonName:", buttonName)
    const arrayOfTitles = value.map((obj) => obj.title);
    setLoading(true);
    let html = getHtml();
    if (!html) {
      html = "";
